import{Ta as o,a as t,b as n,l,v as d}from"./chunk-QAX5D2K5.js";var g="resume-builder-llm-settings",m=`You are an Elite Technical Recruiter and AI-Screening Strategist. Analyze the job description and the candidate's master resume provided below.
...
Analyze the resume thoroughly and return ALL suggestions that would meaningfully improve the candidate's ATS match score or recruiter impression. Each suggestion MUST include a priority score (1\u201310, where 10 is most critical) and an impact level ("critical", "high", "medium", or "low"). Focus on the highest-impact changes first. Stop generating when remaining suggestions would be marginal (priority \u2264 3). Typically this produces 3\u20138 suggestions per pass; exceed 8 only when the resume has significant structural gaps or missing critical skills.

CRITICAL CONSTRAINTS:
- Each resume item (identified by itemId) MUST appear AT MOST ONCE across all suggestions. Do NOT suggest multiple actions on the same item.
- Do NOT include contradictory suggestions (e.g., suggesting to both activate AND deactivate the same skill or section).
- Do NOT repeat suggestions that were already made in earlier messages in this conversation.
...
OUTPUT FORMAT:
Return ONLY a valid JSON object (NOT wrapped in markdown code blocks). Do not include conversational text before or after the object. The object has two keys:
1. "suggestions" \u2014 an array of suggestion objects, ordered by priority (highest first).
2. "remainingAnalysis" \u2014 a brief string summarizing what other improvements you noticed but did not include in this batch. Set to null if the resume is well-optimized and no further changes are recommended.
...
Schema:
{
  "suggestions": [
    {
      ...
      "priority": <1-10 integer, 10 = most critical>,
  ...
  "remainingAnalysis": "string describing additional improvements not included, or null"
}
...
2. MODIFY (Focus on Business Outcomes & Organizational Leadership):
   ...
   - FORBIDDEN verbs: Implemented (task-level), Coded, Debugged (hands-on engineering at this level is typically not highlighted).`};function p(s){return m+v[s]+h}var c={selectedProvider:"openai",configs:{openai:{provider:"openai",apiKey:"",model:"gpt-4o"},anthropic:{provider:"anthropic",apiKey:"",model:"claude-3-5-sonnet-20240620"},gemini:{provider:"gemini",apiKey:"",model:"gemini-1.5-pro"},xai:{provider:"xai",apiKey:"",model:"grok-2"},custom:{provider:"custom",apiKey:"",baseUrl:"http://localhost:11434/v1",model:"llama3"},lmstudio:{provider:"lmstudio",apiKey:"",baseUrl:"http://localhost:1234/v1",model:""}},tierPromptOverrides:{},preferences:t({},o)},u=class s{settings=d(c);constructor(){this.loadSettings()}loadSettings(){try{let e=localStorage.getItem(g);if(e){let i=JSON.parse(e);this.settings.set(n(t(t({},c),i),{configs:t(t({},c.configs),i.configs||{}),tierPromptOverrides:i.tierPromptOverrides||{},preferences:t(t({},o),i.preferences||{})}))}}catch(e){console.error("Failed to parse LLM settings from local storage",e)}}getActiveConfig(){let e=this.settings();return e.configs[e.selectedProvider]}updateProvider(e){this.settings.update(i=>n(t({},i),{selectedProvider:e})),this.saveSettings()}updateConfig(e,i){this.settings.update(r=>{let a=t({},r.configs);return a[e]=t(t({},a[e]),i),n(t({},r),{configs:a})}),this.saveSettings()}getTierPrompt(e){return this.settings().tierPromptOverrides[e]??p(e)}getDefaultTierPrompt(e){return p(e)}hasTierOverride(e){return e in this.settings().tierPromptOverrides}setTierPromptOverride(e,i){this.settings.update(r=>n(t({},r),{tierPromptOverrides:n(t({},r.tierPromptOverrides),{[e]:i})})),this.saveSettings()}resetTierPrompt(e){this.settings.update(i=>{let r=t({},i.tierPromptOverrides);return delete r[e],n(t({},i),{tierPromptOverrides:r})}),this.saveSettings()}getPreferences(){return this.settings().preferences??t({},o)}updatePreference(e,i){this.settings.update(r=>n(t({},r),{preferences:n(t({},r.preferences),{[e]:i})})),this.saveSettings()}saveSettings(){try{localStorage.setItem(g,JSON.stringify(this.settings()))}catch(e){console.error("Failed to save LLM settings",e)}}static \u0275fac=function(i){return new(i||s)};static \u0275prov=l({token:s,factory:s.\u0275fac,providedIn:"root"})};export{u as a};
